...
MIDI_RESPONSE[ANALOG_MESSAGE] = function(board) {
  ...
  if (board.getPin(board.getAnalogPin(pin))) {
    board.getPin(board.getAnalogPin(pin)).value = value;
  }
...
SYSEX_RESPONSE[PIN_STATE_RESPONSE] = function (board) {
  ...
  board.getPin(pin).mode = board.currentBuffer[3];
  board.getPin(pin).state = board.currentBuffer[4];
  if (board.currentBuffer.length > 6) {
    board.getPin(pin).state |= (board.currentBuffer[5] << 7);
  }
  if (board.currentBuffer.length > 7) {
    board.getPin(pin).state |= (board.currentBuffer[6] << 14);
  }
...
SYSEX_RESPONSE[ANALOG_MAPPING_RESPONSE] = function(board) {
  ...
  for (var i = 2; i < board.currentBuffer.length - 1; i++) {
    ...
    board.getPin(pin).analogChannel = currentValue;
    if (currentValue !== 127) {
...
/**
 * Lazily initialize a digital pin
 * @param {pinNumber} the pin number.
 */

Board.prototype.getPin = function(pinNumber) {
  return this.pins[pinNumber] || (this.pins[pinNumber] = {});
};

/**
 * Lazily initialize an analog pin
 * @param {pinNumber} the pin number.
 */

Board.prototype.getAnalogPin = function(pinNumber) {
  return this.analogPins[pinNumber] || (this.analogPins[pinNumber] = {});
};
...
Board.prototype.servoConfig = function(pin, min, max) {
  ...
  this.getPin(pin).mode = this.MODES.SERVO;
  this.transport.write(new Buffer(data));
};
...
Board.prototype.digitalWrite = function(pin, value) {
  ...
  this.getPin(pin).value = value;
  for (var i = 0; i < 8; i++) {
    pinRecord = this.getPin(8 * port + i);
...
Board.prototype.reportAnalogPin = function(pin, value) {
  if (value === 0 || value === 1) {
    this.getPin(this.getAnalogPin(pin)).report = value;
