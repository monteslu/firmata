...
MIDI_RESPONSE[ANALOG_MESSAGE] = function (board) {
    ...
    getPin(board, board.analogPins[port]).value = value;
...
SYSEX_RESPONSE[CAPABILITY_RESPONSE] = function (board) {
    ...
    for (var i = 2, n = 0; i < board.currentBuffer.length - 1; i++) {
        if (board.currentBuffer[i] === 127) {
            ...
            board.sp.write(new Buffer([REPORT_DIGITAL | board.pins.length - 1, 1]));
            supportedModes = 0;
            n = 0;
            continue;
        }
...
SYSEX_RESPONSE[PIN_STATE_RESPONSE] = function (board) {
    ...
    getPin(board, pin).mode = board.currentBuffer[3];
    getPin(board, pin).value = board.currentBuffer[4];
    if (board.currentBuffer.length > 6) {
        getPin(board, pin).value |= (board.currentBuffer[5] << 7);
    }
    if (board.currentBuffer.length > 7) {
        getPin(board, pin).value |= (board.currentBuffer[6] << 14);
    }
...
SYSEX_RESPONSE[ANALOG_MAPPING_RESPONSE] = function (board) {
    ...
    for (var i = 2; i < board.currentBuffer.length - 1; i++) {
        ...
        getPin(board, pin).analogChannel = currentValue;
        if (currentValue !== 127) {
            board.analogPins.push(pin);
            board.sp.write(new Buffer([REPORT_ANALOG | board.analogPins.length - 1, 1]));
        }
        pin++;
    }
...
function getPin(board, pinNum){
    ...
    board.pins[pinNum] = board.pins[pinNum] || {};
    return board.pins[pinNum];
}

function getAnalogPin(board, pinNum){
    board.analogPins = board.analogPins || [];
    board.analogPins[pinNum] = board.analogPins[pinNum] || {};
    return board.analogPins[pinNum];
}
...
var Board = function(port, options, callback) {
    ...
    if(typeof port === 'object'){
        ...
        console.log('no serial port passed into contstructor, and no SerialPort dependency available');
        throw new Error('no SerialPort class defined');
    }
...
Board.prototype.digitalWrite = function (pin, value) {
    ...
    getPin(this, pin).value = value;
    for (var i = 0; i < 8; i++) {
        //var pinObj = getPin(this, 8 * port + i);
        //console.log('digitalWrite', pinObj, value);
        if (getPin(this,[8 * port + i]).value) {
            portValue |= (1 << i);
        }
    }
...
Board.prototype.reportAnalogPin = function (pin, value) {
    if(value === 0 || value === 1) {
        getPin(this,this.analogPins[pin]).report = value;
